...
my %sec_headers = (
    ...
    -X_XSS_Protection            => '0',
    -Content_Security_Policy     => "upgrade-insecure-requests; default-src 'self'; script-src 'none'; style-src 'none'; font-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; connect-src 'none'; form-action 'self';",
...
if ($filename =~ /\.(?:pl|cgi|php\d*|phps|pht|phar|py|sh|bash|zsh|exe|bat|cmd|html?|js|mjs|cjs|shtml|phtml|svg|svgz|xml|mhtml|asp[x]?|jspx?|vbs|vba|ps1|wasm|xhtml|conf|config|jar|war|ear|swf|hta|scr|com|msi|vbe|jse|wsf|wsh|lnk|reg|jnlp|pif|desktop|url|application|gadget|msu|msp|docm|dotm|xlsm|xltm|pptm|potm|ppsm|hlp|chm|ade|adp|mde|msc|mst|sct|shb|shs|wsc)(?:\.|\z)/i) {
