...
# Test 1: CSP parsing with semicolons and colons
{
    ...
    my $env = "PERL5LIB=extlib/lib/perl5 UPLOAD_SECURITY_HEADERS=\"Content-Security-Policy: $csp\" REQUEST_METHOD=GET";
...
# Test 2: Header normalization (case and dash/underscore)
{
    ...
    my $env = "PERL5LIB=extlib/lib/perl5 UPLOAD_SECURITY_HEADERS=\"$headers\" REQUEST_METHOD=GET";
    my $output = `$env perl $cgi_script 2>&1`;
